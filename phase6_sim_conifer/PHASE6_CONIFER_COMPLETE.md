...
> Sign-off gate for `phase5_fpga_conifer` (which is also COMPLETE — bitstream built).

This is the GBDT analog of `phase6_sim/functional/` (the honest, real-IP MLP
...
generator writes it and `phase5_fpga_conifer/hdl/sigmoid_rom.v` `$readmemh`s the
...
Contrast with the MLP: `phase6_sim/functional/check_results.py` needed a ±3-count
...
`gen_vectors.py` lives in `phase6_sim/functional/`, not phase 4.
...
```bash
# 1. Regenerate the chain golden (WSL, after phase4_conifer/make_tb_data.py)
...
\\wsl.localhost\Ubuntu\home\younix\nfl-fpga-accelerator\phase6_sim_conifer\run_xsim_gbdt.bat 100

REM 3. Full-UART test (Windows). Self-checking.
\\wsl.localhost\Ubuntu\home\younix\nfl-fpga-accelerator\phase6_sim_conifer\run_xsim_uart_gbdt.bat
```

```bash
# 4. Gate the regression (WSL)
python phase6_sim_conifer/check_results.py     # exit 0 = PASS
```
...
| Gap | Assessment |
|---|---|
| No leaf-level unit tests for `uart_framing_conifer.v` | It is new HDL, covered only end-to-end by Layer 3 (including the NACK and SOF-collision paths — the two things unit tests would target). `uart_rx.v` / `uart_tx.v` are **unchanged** and still covered by the MLP's cocotb unit tests in `phase6_sim/cocotb/`. |
...
equality vs a tolerance band). This gated `phase5_fpga_conifer`, which is also
complete: 12,095 LUTs (58%), 0 DSPs, WNS +0.965 ns, bitstream built.

**Next:** `phase7_deploy_conifer/` — pyserial harness for the 65-byte packet on
