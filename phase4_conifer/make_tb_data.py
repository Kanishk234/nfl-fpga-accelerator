...
Run from project root (WSL) after convert_hls.py:
    python phase4_conifer/make_tb_data.py
"""
...
def cpp_golden(booster, name, X):
    ...
    cfg['OutputDir'] = f'phase4_conifer/scan_{name}'  # reuse scan dir
...
def write_tb(project, X):
    path = f'phase4_conifer/{project}/tb_data/tb_input_features.dat'
...
if __name__ == '__main__':
    ...
    np.save('phase4_conifer/hls_win/golden_cpp.npy', g)
    ...
    np.save('phase4_conifer/hls_spread/golden_cpp.npy', g)
    print(f"  golden_cpp.npy: residual range [{g.min():.3f}, {g.max():.3f}]")

    print("\nNext: phase4_conifer/run_synthesis.bat (Windows)")
