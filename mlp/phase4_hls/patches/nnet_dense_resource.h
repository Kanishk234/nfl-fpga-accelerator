...
void dense_resource_rf_leq_nin(data_T data[CONFIG_T::n_in], res_T res[CONFIG_T::n_out],
    ...
    if (CONFIG_T::reuse_factor > 1) {
        // ROM_nP_BRAM is deprecated in Vitis HLS 2020.2+; bind_storage is the correct 2025.2 syntax
...
Result:
    for (int ires = 0; ires < CONFIG_T::n_out; ires++) {
        // PIPELINE II=1 instead of UNROLL: with io_serial the output is an hls::stream<>.
        // UNROLL tries to write all n_out elements simultaneously, but a stream only accepts
        // one write per cycle — HLS generates a huge n_out-to-depth drain mux (~7,620 extra
        // LUT for dense_1). PIPELINE II=1 writes one element per cycle: simple counter, ~50 LUT.
...
Result:
    for (int ires = 0; ires < CONFIG_T::n_out; ires++) {
        // PIPELINE II=1 instead of UNROLL — see rf_leq_nin comment above.
        #pragma HLS PIPELINE II=1
        res[ires] = cast<data_T, res_T, CONFIG_T>(acc[ires]);
    }
}
