...
## Architecture — `mlp/phase7_deploy/`
...
- **`ui/webapp.py` + `ui/index.html` — local web app** (the nicer-looking one). The backend
  ...
  as an inline message. Launch: `python mlp/phase7_deploy/ui/webapp.py` → auto-opens
  `http://127.0.0.1:8713`.
...
`mlp/phase6_sim/test_vectors/input_games.csv` + `expected_outputs.csv`. That directory was
deleted in the Phase 6 post-audit cleanup; the authoritative vectors live in
`mlp/phase6_sim/functional/` with a different schema (a hex `$readmemh` file, not a feature-per-
column CSV). Run as-is, both scripts crash.

**Fix:** Repointed both to `mlp/phase6_sim/functional/`:
...
python mlp/phase7_deploy\ui\app.py
```
...
| Validation scripts crash | Point at `test_vectors/` deleted in Phase 6 cleanup | Repoint to `mlp/phase6_sim/functional/` (`tb_inputs.mem`/`sim_results.csv`) |
