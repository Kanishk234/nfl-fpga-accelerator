...
in `phase5_fpga/hdl/uart_framing.v`. The FSM now advances when `tx_done` fires
...
**Fix:** Created `phase6_sim/stubs/mlp_fifo_deep.v` with depth-256 replacements for
...
**Fix:** Created `phase6_sim/stubs/mlp_fifo_dual_w8.v` — a "replay-once" FIFO that stores
...
**Fix:** Created `phase6_sim/stubs/mlp_fifo_replay_w8.v` with:
...
cd /home/younix/nfl-fpga-accelerator/phase6_sim/cocotb
...
python3 /home/younix/nfl-fpga-accelerator/phase6_sim/print_report.py
...
verification. That lives in **`phase6_sim/functional/`** (see its README).
