...
> Board deploy is phase 7 (`gbdt/phase7_deploy/`).

This is the GBDT analog of `mlp/phase5_fpga/` (MLP UART integration). The MLP
...
`mlp/phase5_fpga/hdl/` unchanged (single source of truth, not copied).
...
`gbdt/phase6_sim/run_xsim_gbdt.bat` drives controller + ROM + **both real
...
`gbdt/phase6_sim/run_xsim_uart_gbdt.bat` (`tb_top_uart_gbdt.v`) drives the
...
| | GBDT (this design) | MLP (`mlp/phase5_fpga`) |
|---|---|---|
...
| Step | What | Gate |
|---|---|---|
| Board deploy | `gbdt/phase7_deploy/` pyserial harness (new 65-byte packet, COM8) | bit-exact vs golden on hardware; honest UART-bound latency framing |
