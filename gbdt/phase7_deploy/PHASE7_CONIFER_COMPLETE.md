...
This is the GBDT analog of `mlp/phase7_deploy/` (MLP board deployment). Everything
new lives here per the `_conifer` sibling-folder convention; the one exception
is `mlp/phase7_deploy/ui/index.html`, which is now **shared** by both UIs
...
| | MLP (`mlp/phase7_deploy`) | GBDT (this folder) |
|---|---|---|
...
Protocol source of truth: `gbdt/phase5_fpga/hdl/uart_framing_conifer.v`.
...
`mlp/phase5_fpga/hdl/`, not copied):
...
- **`GameCatalog` is imported as-is** from `mlp/phase7_deploy/inference/`; it is a
...
# 4. Web UI
python gbdt/phase7_deploy\ui\webapp_gbdt.py        # http://127.0.0.1:8714
```
...
2. **It is monotonic in `elo_diff`** (verified, no inversions). The win head was
   trained with monotone constraints (`gbdt/phase2_train`), and that training-time
