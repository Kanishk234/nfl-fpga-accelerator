...
This is the GBDT analog of `mlp/phase4_hls/` (hls4ml for the MLP). conifer is
...
```
gbdt/phase4_hls/precision_scan.tsv       — scan results (committed)
gbdt/phase4_hls/hls_win/                 — generated HLS project + exported IP (gitignored)
gbdt/phase4_hls/hls_spread/              — generated HLS project + exported IP (gitignored)
gbdt/phase4_hls/hls_*/golden_cpp.npy     — bit-accurate C++ goldens (verification reference)
gbdt/phase4_hls/hls_*/vivado_synth.rpt   — real utilization reports
```
...
```bash
# WSL:
python gbdt/phase4_hls/smoke_test.py       # needs one-time header fix (see step 1)
python gbdt/phase4_hls/precision_scan.py   # appends to precision_scan.tsv
python gbdt/phase4_hls/convert_hls.py      # emit + patch both HLS projects
python gbdt/phase4_hls/make_tb_data.py     # testbench vectors + goldens
```
```bat
:: Windows:
gbdt/phase4_hls\run_synthesis.bat          :: csim+csynth+cosim+IP export, both stages
gbdt/phase4_hls\run_vsynth.bat             :: real LUT/FF numbers
```
...
- **Generated projects gitignored** — regenerable from committed models +
  scripts, same convention as `mlp/phase4_hls/hls_project/`.
...
phases get `gbdt/phase5_fpga/`, `gbdt/phase6_sim/`, `gbdt/phase7_deploy/`
— the MLP originals stay untouched for the 1:1 comparison.

**Done ahead of phase 5** (lives in `gbdt/phase6_sim/` per the MLP
...
`gbdt/phase6_sim/chain_golden/`.

| Step | What | Gate |
|---|---|---|
| Two-stage wrapper (`gbdt/phase5_fpga/`) | Chain stage1 → sigmoid ROM → stage2, `+ vegas_spread` adder, AXIS interface matching the phase 5 UART plumbing | XSIM bit-exact vs `gbdt/phase6_sim/chain_golden/` |
| Vivado implementation | Full impl of the combined design | Timing met; fits with UART wrapper |
| Board deploy (`gbdt/phase7_deploy/`) | Reuse phase 7 pyserial harness; head-to-head latency + resource table vs MLP | Bit-exact vs golden; honest UART-bound framing |
