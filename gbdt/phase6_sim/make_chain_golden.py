...
Lives in gbdt/phase6_sim/ (not gbdt/phase4_hls/) per the MLP precedent:
golden generation for the wrapper regression is verification work
(mlp/phase6_sim/functional/gen_vectors.py + golden.csv). The per-stage cosim
goldens stay in gbdt/phase4_hls/make_tb_data.py — those gate the HLS flow.
...
Run from project root (WSL) after gbdt/phase4_hls/make_tb_data.py:
...
OUT_DIR = 'gbdt/phase6_sim/chain_golden'
...
def cpp_model(model_path, cls, name):
    ...
    cfg['OutputDir'] = f'gbdt/phase4_hls/scan_{name}'  # reuse phase-4 compiled scan dir
...
if __name__ == '__main__':
    ...
    print("Next: gbdt/phase5_fpga wrapper HDL, XSIM'd against these goldens.")
