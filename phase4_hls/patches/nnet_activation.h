...
template <class data_T, class res_T, typename CONFIG_T> void relu(data_T data[CONFIG_T::n_in], res_T res[CONFIG_T::n_in]) {
    // No #pragma HLS PIPELINE here — sequential loop shares 1 comparator across all n_in iterations.
    // With PIPELINE the loop is fully unrolled (128/64/32 parallel comparators = 8,736 LUT on dense_1-3).
    // Without PIPELINE, HLS infers a sequential II=1 loop: ~10 LUT per layer vs ~4,992/2,496/1,248.
    // myproject.cpp calls are sequential (no HLS DATAFLOW), so no FIFO depth issue.
